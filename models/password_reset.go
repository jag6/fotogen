...
type PasswordReset struct {
	...
	TokenHash string
	ExpiresAt time.Time
	// token only set when creating a new password reset
}
...
func (prs *PasswordResetService) Create(email string) (*PasswordReset, error) {
	// verify we have a valid email for user and get user's id
...
func (prs *PasswordResetService) Consume(token string) (*User, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("conusme: %w", err)
	}
	return &user, nil
}
